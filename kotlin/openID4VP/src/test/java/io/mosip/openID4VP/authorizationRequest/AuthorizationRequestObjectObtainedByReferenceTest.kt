...
class AuthorizationRequestObjectObtainedByReference {
    ...
    private val validEncodedAuthorizationRequestWithRequestUriSupport =
    ...
    fun `should return Authorization Request if it has request uri and it is a valid authorization request`() {
        ...
        } returns validAuthorizationRequestObject
    ...
    @Test
    fun `should throw exception when the call to request_uri method fails`() {
        every {
            NetworkManagerClient.sendHTTPRequest(
                "https://verifier/verifier/get-auth-request-obj",
                HTTP_METHOD.GET
        ...
        val exceptionWhenRequestUriNetworkCallFails = assertThrows(Exception::class.java) {
            AuthorizationRequest.validateAndGetAuthorizationRequest(
                validEncodedAuthorizationRequestWithRequestUriSupport,
                { _: String -> },
                trustedVerifiers,
                false
            )
        ...
        assertEquals("VP sharing failed due to connection timeout",exceptionWhenRequestUriNetworkCallFails.message)
    }
